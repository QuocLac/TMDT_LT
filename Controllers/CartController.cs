...
namespace TMDT_LT.Controllers
{
    public class CartController : Controller
    {
        ...
        public CartController(ApplicationDbContext context, PromotionEngine promotionEngine, ICrossSellAprioriService crossSellAprioriService)
        {
            _context = context;
        ...
        public async Task<IActionResult> Index()
        {
            ...
            await AttachCrossSellRecommendationsAsync(cart);
        ...
        private async Task RefreshCartMetadataAsync(List<CartItemVM> cart)
        {
            ...
            foreach (var item in cart)
            {
                ...
                if (variant != null)
                {
                    ...
                    item.ProductName = variant.Product?.Name ?? item.ProductName;
                    item.ImageUrl = !string.IsNullOrEmpty(variant.ImageUrl) ? variant.ImageUrl : (variant.Product?.MainImage ?? item.ImageUrl);
                    item.Color = variant.Color ?? item.Color;
                    item.Storage = variant.Storage ?? item.Storage;
        ...
        private async Task AttachCrossSellRecommendationsAsync(List<CartItemVM> cart)
        {
            var productIds = cart.Where(x => x.ProductId > 0).Select(x => x.ProductId).Distinct().ToList();
            if (!productIds.Any()) return;

            var map = await _crossSellAprioriService.GetRecommendationsForProductsAsync(
                productIds,
            ...
            foreach (var item in cart)
            {
                item.CrossSellRecommendations = item.ProductId > 0 && map.TryGetValue(item.ProductId, out var list)
                    ? list
                    : new List<CrossSellRecommendationVM>();
            }
        }
        // =======================================================
